...
def launch_gui() -> int:
    ...
    class STLPreviewApp:
        ...
        def _build_layout(self):
            ...
            ttk.Label(nav_frame, text="Ordnerstruktur", style="SectionTitle.TLabel").pack(anchor="w", pady=(0, 6))
        ...
        def _save_scan_cache(self, summary: ScanSummary, directories: list[Path]):
            try:
                rel_dirs = []
        ...
        def _try_load_scan_cache(self) -> tuple[ScanSummary, list[Path]] | None:
        ...
        def _initial_scan_worker(self):
            try:
                ...
                self.ui_queue.put(("scan_complete", summary, directories))
        ...
        def _handle_ui_event(self, event: tuple):
            ...
            if kind == "dirs_ready":
                ...
                if not self.render_running:
                    self._clear_activity()
                self._save_scan_cache(self.summary, directories)
                self._append_log(
        ...
        def open_config_dialog(self):
            ...
            ttk.Label(frame, text="Blender-Pfad (optional)").grid(row=16, column=0, sticky="w", pady=4)
            ttk.Entry(frame, textvariable=blender_var, width=56).grid(
                row=17, column=0, sticky="we", pady=(0, 6)
            )
